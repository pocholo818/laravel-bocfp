...
<div class="row">
    <div class="col-sm-12">
        ...
        <div class="card">
            ...
            <form class="row g-3">
                ...
                <div class="col-md-2">
                    ...
                    {!! html()->select('sex', $sexes, $selected_sex)->id('input_sex')->class('form-control') !!}
                </div>
                <div class="col-md-2">
                ...
                <div class="col-12">
                    ...
                    <a href="{{ route('backoffice.child.index') }}" class="btn btn-light" type="submit">Reset Filter</a>
                </div>
            </form>
            </div>
        </div>
        ...
        <form id="export_form" action="{{-- route('backoffice.child.export') --}}" method="POST">
            {!! csrf_field() !!}
        ...
        <div class="card-header align-items-center d-flex">
            ...
            <div class="form-check form-switch form-switch-right form-switch-md">
                ...
                <div class="d-none d-md-flex gap-2">
                    @if($auth->canAny(['backoffice.child.create'],'admin'))
                        <a href="{{ route('backoffice.child.create') }}" class="btn btn-secondary" type="submit"><i class="fa-solid fa-plus"></i> Add New Child</a>
                    @endif
                    {{-- @if($auth->canAny(['backoffice.child.export'],'admin')) --}}
                ...
                <div class="d-md-none">
                    <div class="btn-group" role="group">
                        ...
                        <ul class="dropdown-menu">
                            <li> 
                                @if($auth->canAny(['backoffice.child.create'],'admin'))
                                    <a href="{{ route('backoffice.child.create') }}" class="dropdown-item active">Add New Child</a>
                                @endif
                                {{-- @if($auth->canAny(['backoffice.child.export'],'admin')) --}}
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group" role="group">
                                    <button class="btn btn-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">Action</button>
                                    <ul class="dropdown-menu">
                                        <li><a class="dropdown-item active" href="{{ route('backoffice.child.show', $record->id) }}">View Details</a>
                                        @if($auth->canAny(['backoffice.child.edit'],'admin') AND $record->status == "active")
                                            <li><a class="dropdown-item active" href="{{route('backoffice.child.edit', $record->id)}}">Edit Details</a></li>
                                        @endif
                                        @if($auth->canAny(['backoffice.child.update_status'],'admin'))
                                            <li><a class="dropdown-item active btn-update-status" href="#" data-status="{{ $record->status }}" data-url="{{route('backoffice.child.update_status', $record->id)}}">{{ $record->status == "inactive" ? "Activate" : "Deactivate" }}</a></li>
                                        @endif
                                    </ul>
                                </div>
                            </td>
